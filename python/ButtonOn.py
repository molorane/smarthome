...
class ButtonOn(threading.Thread):
    ...
    def run(self):
        while True:
            if not GPIO.input(self.btnID):
                ...
                self.lcd.print(str)
                print(str)
                threading.Thread(target=self.onOutSideLights).start()
                time.sleep(0.2)
    ...
    def onOutSideLights(self):
        conn = ConnectionDB()
        data = conn.getAllOutSideLights()
        devices = []
        for app in data:
            devices.append(app['conn_d_id'])
        macros = MacroCommand(devices, 'on', self.lcd, 'system')
        macros.execute()
